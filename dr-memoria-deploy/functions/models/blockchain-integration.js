...
const CONTENT_REGISTRY_ABI = [
  {
    "inputs": [
      ...
      { "internalType": "uint8", "name": "humanContributionPercent", "type": "uint8" }
    ],
...
class BlockchainIntegration {
  constructor() {
    ...
    this.web3 = new Web3(process.env.ETHEREUM_RPC_URL || functions.config().ethereum.rpc_url);
    
    // Contract addresses for different networks
    this.contractAddresses = {
      mainnet: process.env.CONTENT_REGISTRY_MAINNET || functions.config().ethereum.mainnet_contract,
      polygon: process.env.CONTENT_REGISTRY_POLYGON || functions.config().ethereum.polygon_contract,
  ...
  updateNetwork(network, rpcUrl) {
    try {
      ...
      handleError(error, ErrorTypes.BLOCKCHAIN_ERROR, 'Failed to update blockchain network');
      return false;
    }
  }
  ...
  generateContentHash(content) {
    try {
      ...
      handleError(error, ErrorTypes.PROCESSING_ERROR, 'Failed to generate content hash');
      return null;
    }
  }
  ...
  async storeMetadataOnIPFS(content, creativePassport) {
    try {
      ...
      const metadata = {
        name: content.title,
        description: content.description || 'Creative work registered on Dr. Memoria\'s Anthology',
      ...
      handleError(error, ErrorTypes.STORAGE_ERROR, 'Failed to store metadata on IPFS');
      return null;
    }
  }
  ...
  async registerOnBlockchain(content, creativePassport, privateKey) {
    try {
      ...
      const metadataURI = await this.storeMetadataOnIPFS(content, creativePassport);
      if (!metadataURI) {
      ...
      const humanContributionPercent = Math.round(creativePassport.humanContributionPercentage * 100);
      ...
      handleError(error, ErrorTypes.BLOCKCHAIN_ERROR, 'Failed to register content on blockchain');
      return {
        success: false,
        error: error.message
      };
  ...
  async verifyContent(contentHash) {
    try {
      ...
      handleError(error, ErrorTypes.BLOCKCHAIN_ERROR, 'Failed to verify content on blockchain');
      return {
        verified: false,
        error: error.message
      };
  ...
  async generateVerificationQR(contentHash, registrationData) {
    try {
      ...
      handleError(error, ErrorTypes.PROCESSING_ERROR, 'Failed to generate verification QR code');
      return null;
    }
  }
  ...
  async createBlockchainCertification(content, creativePassport, privateKey) {
    try {
      ...
      if (!registrationResult.success) {
        throw new Error(`Blockchain registration failed: ${registrationResult.error}`);
      }
      ...
      const certification = {
        ...
        humanContributionPercentage: creativePassport.humanContributionPercentage,
      ...
      logger.info(`Blockchain certification created for content: ${content.title}`);
      ...
      handleError(error, ErrorTypes.BLOCKCHAIN_ERROR, 'Failed to create blockchain certification');
      return {
        success: false,
        error: error.message
      };
