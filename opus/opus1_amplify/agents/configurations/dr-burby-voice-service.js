    ...
    return cors(req, res, async () => {
      try {
        ...
        if (pitch < -20.0 || pitch > 20.0) {
          return res
            .status(400)
            .send({ error: 'Pitch must be between -20.0 and 20.0' });
        }

        if (speakingRate < 0.25 || speakingRate > 4.0) {
          return res
            .status(400)
            .send({ error: 'Speaking rate must be between 0.25 and 4.0' });
        }

        // Log the request (remove sensitive info in production)
        console.log(
          `Generating speech for text of length: ${text.length} characters`
        );
        ...
        res.set(
          'Content-Disposition',
          'attachment; filename=dr-burby-voice.mp3'
        );
        ...
        res.sendFile(
          ...
          err => {
            if (err) {
              ...
              return res
                .status(500)
                .send({ error: 'Error delivering audio file' });
            }
