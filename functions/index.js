...
const loadConfig = () => {
  try {
    const configPath = path.join(
      __dirname,
      '../infrastructure/vision_lake/config.yaml'
    );
...
const validateAuth = async (req, res, next) => {
  try {
    ...
    if (
      config.integration &&
      config.integration.authentication &&
      config.integration.authentication.roles
    ) {
...
app.post('/vision/analyze', validateAuth, async (req, res) => {
  try {
    const { image, features = ['LABEL_DETECTION', 'TEXT_DETECTION'] } =
      req.body;
    ...
    await firestoreClient
      .collection(config.integration.firestore.collection)
      .add({
        result,
        userId: req.user.uid,
    ...
    return res
      .status(500)
      .json({ error: `Vision API Error: ${error.message}` });
  }
});
...
app.post('/vertexai/predict', validateAuth, async (req, res) => {
  try {
    ...
    if (!endpoint || !instances) {
      return res
        .status(400)
        .json({ error: 'Endpoint and instances are required' });
    }

    // Get endpoint configuration
    const endpointConfig = Object.entries(config.vertex_ai.endpoints).find(
      ([key, value]) => key === endpoint || value.display_name === endpoint
    );

    if (!endpointConfig) {
      return res
        .status(400)
        .json({ error: `Endpoint ${endpoint} not found in configuration` });
    }
  ...
  try {
    ...
    if (!endpointConfig) {
      return res
        .status(400)
        .json({
          error: `Endpoint ${modelEndpoint} not found in configuration`,
        });
    }
    ...
    return res
      .status(500)
      .json({ error: `Multimodal Analysis Error: ${error.message}` });
  }
});
  ...
  .onRun(async context => {
    try {
      ...
      for (const collection of collections) {
        const expiredDocs = await firestoreClient
        ...
        if (count > 0) {
          await batch.commit();
          console.log(
            `Cleaned up ${count} expired documents from ${collection}`
          );
        }
      }

      // Clean temporary storage files
      const buckets = [config.integration.storage_bucket, 'aixtiv-tts-output'];
      ...
      for (const bucketName of buckets) {
        ...
        if (deletedCount > 0) {
          console.log(
            `Deleted ${deletedCount} old temporary files from ${bucketName}`
          );
        }
      }
 ...
 */
exports.monitorVisionUsage = functions.pubsub
    ...
    try {
      ...
      snapshot.forEach(doc => {
        ...
        if (data.result && data.result.labelAnnotations) {
          featureCounts.LABEL_DETECTION =
            (featureCounts.LABEL_DETECTION || 0) + 1;
        }
        if (data.result && data.result.textAnnotations) {
          featureCounts.TEXT_DETECTION =
            (featureCounts.TEXT_DETECTION || 0) + 1;
        }
        if (data.result && data.result.faceAnnotations) {
          featureCounts.FACE_DETECTION =
            (featureCounts.FACE_DETECTION || 0) + 1;
        }
      ...
      if (snapshot.size > 1500) {
        // 1800 requests per minute limit, alert at 1500
        // Notify admins
        const adminTopic = pubsubClient.topic('vision-alerts');
        await adminTopic.publish(
          Buffer.from(
            JSON.stringify({
              timestamp,
              alert: 'HIGH_USAGE',
...
exports.setupConfig = functions.https.onRequest(async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: `Setup Config Error: ${error.message}` });
  }
});
