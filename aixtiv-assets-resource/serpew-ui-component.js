...
export const SERPEWDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const assessment = await q4dService.getLatestHOMBDIHOAssessment(
          authState.userId
        );
        setAssessmentData(assessment);

        // Fetch market position analysis
        const position = await q4dService.getLatestMarketPositionAnalysis(
          authState.userId
        );
  ...
  const prepareRiasecRadarData = riasecData => {
    ...
    return [
      {
        subject: 'Realistic',
        A: riasecData.integrated_profile.R,
        fullMark: 100,
      },
      {
        subject: 'Investigative',
        A: riasecData.integrated_profile.I,
        fullMark: 100,
      },
      {
        subject: 'Artistic',
        A: riasecData.integrated_profile.A,
        fullMark: 100,
      },
      { subject: 'Social', A: riasecData.integrated_profile.S, fullMark: 100 },
      {
        subject: 'Enterprising',
        A: riasecData.integrated_profile.E,
        fullMark: 100,
      },
      {
        subject: 'Conventional',
        A: riasecData.integrated_profile.C,
        fullMark: 100,
      },
    ];
  };
    ...
    <div className="bg-white shadow rounded-lg p-6">
      ...
      <div className="h-72">
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart
            data={prepareRiasecRadarData(
              assessmentData.results.riasec_validation
            )}
          >
            <PolarGrid />
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-2">
            Your RIASEC profile has been validated through a dual-assessment
            process that compares theoretical Holland Code results with your
        ...
        <div>
          ...
          <p className="text-sm text-gray-600 mt-2">
            Your RIASEC profile shows strongest alignment with Investigative and
            Enterprising traits, indicating a preference for analytical
            problem-solving combined with leadership and influence.
          </p>
          <p className="text-sm text-gray-600 mt-2">
            This pattern is typically seen in roles like business analyst,
            management consultant, and research director.
          </p>
        </div>
      </div>
    </div>
    ...
    <div className="bg-white shadow rounded-lg p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <p className="text-sm text-gray-600 mt-2">
            Based on alignment between your RIASEC profile and current role
            requirements
          </p>
        </div>
        <div className="bg-gray-50 p-4 rounded-lg">
      ...
      <div className="mt-8">
        ...
        <ul className="list-disc pl-5 space-y-2">
          {assessmentData.results.role_fit.development_opportunities.map(
            (area, idx) => (
      ...
      <div className="mt-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {Object.entries(
            assessmentData.results.role_fit.alignment_details
          ).map(([key, value]) => (
    ...
    <div className="bg-white shadow rounded-lg p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <div className="text-3xl font-bold text-indigo-600">
            {
              marketPosition.results.individual_analysis
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <div className="flex flex-wrap gap-2">
            {Object.entries(marketPosition.results.jurisdictional_scope).map(
              ([key, value]) => (
                <span
                  key={key}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
        <div>
          ...
          <ul className="list-disc pl-5">
            {marketPosition.results.individual_analysis.relative_strengths.map(
              (strength, idx) => (
                <li key={idx} className="text-sm text-gray-700">
                  {strength}
                </li>
        ...
        <div>
          ...
          <ul className="list-disc pl-5">
            {marketPosition.results.individual_analysis.development_areas.map(
              (area, idx) => (
                <li key={idx} className="text-sm text-gray-700">
                  {area}
                </li>
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
              <div>
                ...
                <ul className="list-disc pl-5">
                  {marketPosition.results.enterprise_analysis.competitive_strengths.map(
                    (strength, idx) => (
                      <li key={idx} className="text-sm text-gray-700">
                        {strength}
                      </li>
              ...
              <div>
                ...
                <ul className="list-disc pl-5">
                  {marketPosition.results.enterprise_analysis.development_areas.map(
                    (area, idx) => (
                      <li key={idx} className="text-sm text-gray-700">
                        {area}
                      </li>
      ...
      <div className="bg-white shadow rounded-lg p-6">
        ...
        <div className="mt-8">
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart
                data={prepareSatisfactionTrendsData(satisfactionTrends)}
              >
                ...
                <Line
                  type="monotone"
                  dataKey="compensationSatisfaction"
                  stroke="#82ca9d"
                  name="Compensation"
                />
                <Line
          ...
          <p className="mt-4 text-sm text-gray-500">
            This chart shows historical satisfaction trends for professionals in
            your role based on national satisfaction measures.
          </p>
        </div>
      </div>
    );
