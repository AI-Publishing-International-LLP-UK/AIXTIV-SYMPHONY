...
describe('DrLucyFormTokenManager', () => {
  ...
  let formTokenManager: DrLucyFormTokenManager;
  let mockCsrfProtection: GatewayCsrfProtection;
  let mockRateLimiter: InMemoryRateLimiter;
  let mockOriginValidator: FormOriginValidator;
  
  // Sample test data
  const testPayload: FormTokenPayload = {
  ...
  beforeEach(() => {
    ...
    (fs.readFileSync as jest.Mock).mockReturnValue(JSON.stringify(mockServiceAccountKey));
    ...
    (crypto.createSign as jest.Mock).mockReturnValue({
      update: jest.fn(),
    ...
    } as unknown as GatewayCsrfProtection;
    
    mockRateLimiter = {
      checkRateLimit: jest.fn().mockResolvedValue(true)
    } as unknown as InMemoryRateLimiter;
    ...
    } as unknown as FormOriginValidator;
    ...
    (formTokenManager as any).createSignedJwt = jest.fn().mockReturnValue('mock-jwt-token');
    (formTokenManager as any).verifyToken = jest.fn().mockReturnValue({
    ...
    (formTokenManager as any).cacheToken = jest.fn();
    (formTokenManager as any).startTokenCleanup = jest.fn();
    (formTokenManager as any).stopTokenCleanup = jest.fn();
  });
  ...
  describe('generateToken', () => {
    it('should return false for invalid token', async () => {
      // Arrange
      (formTokenManager as any).verifyToken = jest.fn().mockReturnValue(null);
      ...
      expect((formTokenManager as any).verifyToken).toHaveBeenCalledWith('invalid-token');
    });
    
    it('should handle errors during revocation', async () => {
      // Arrange
      (formTokenManager as any).verifyToken = jest.fn().mockImplementation(() => {
  ...
  describe('createTokenValidationMiddleware', () => {
    ...
    it('should call next() for valid token and origin', async () => {
      // Arrange
      (formTokenManager.extractTokenFromRequest as jest.Mock).mockReturnValue('valid-token');
      (formTokenManager.validateToken as jest.Mock).mockResolvedValue(true);
    ...
    it('should return 403 for invalid origin', async () => {
      // Arrange
      (mockOriginValidator.validateOrigin as jest.Mock).mockReturnValue(false);
    ...
    it('should return 401 when no token is provided', async () => {
      // Arrange
      (formTokenManager.extractTokenFromRequest as jest.Mock).mockReturnValue(null);
    ...
    it('should return 401 for invalid token', async () => {
      // Arrange
      (formTokenManager.extractTokenFromRequest as jest.Mock).mockReturnValue('invalid-token');
      (formTokenManager.validateToken as jest.Mock).mockResolvedValue(false);
    ...
    it('should return 500 when an error occurs', async () => {
      // Arrange
      (formTokenManager.extractTokenFromRequest as jest.Mock).mockReturnValue('valid-token');
      (formTokenManager.validateToken as jest.Mock).mockRejectedValue(new Error('Validation error'));
  ...
  describe('GatewayCsrfProtection', () => {
    let csrfProtection: GatewayCsrfProtection;
  ...
  describe('FormOriginValidator', () => {
    let originValidator: FormOriginValidator;
    ...
    describe('createOriginValidationMiddleware', () => {
      ...
      expect((formTokenManager as any).createSignedJwt).toHaveBeenCalled();
      expect((formTokenManager as any).cacheToken).toHaveBeenCalled();
    });
    ...
    it('should throw error when rate limit is exceeded', async () => {
      ...
      expect(mockRateLimiter.checkRateLimit).toHaveBeenCalledWith(testPayload.userId);
      expect((formTokenManager as any).createSignedJwt).not.toHaveBeenCalled();
    });
  ...
  describe('validateToken', () => {
    it('should validate a valid token', async () => {
      ...
      // Assert
      expect(result).toBe(true);
      expect((formTokenManager as any).verifyToken).toHaveBeenCalledWith('valid-token');
    });
  ...
  describe('validateTokenWithCsrf', () => {
    ...
    it('should reject with invalid CSRF token', async () => {
      ...
      const spyValidateToken = jest.spyOn(formTokenManager, 'validateToken')
        .mockResolvedValue(true);
      (mockCsrfProtection.validateCsrfToken as jest.Mock).mockReturnValue(false);
  ...
  describe('revokeToken', () => {
    it('should successfully revoke a valid token', async () => {
      ...
      (formTokenManager as any).tokenCache = mockTokenCache;
      ...
      // Assert
      expect(result).toBe(true);
      expect((formTokenManager as any).verifyToken).toHaveBeenCalledWith(mockToken);
