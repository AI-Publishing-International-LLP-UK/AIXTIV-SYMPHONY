...
export class FirebaseInitializer {
  ...
  public getServices(): FirebaseServices {
    if (!this.services) {
      throw new Error(
        'Firebase services not initialized. Call initialize() first.'
      );
    }
    return this.services;
  }
  ...
  private setupEmulators(
    ...
    if (ports.firestore) {
      ...
      console.log(
        `Connected to Firestore emulator at ${host}:${ports.firestore}`
      );
    }
    ...
    if (ports.functions) {
      ...
      console.log(
        `Connected to Functions emulator at ${host}:${ports.functions}`
      );
    }
