...
class WFASwarmQueue {
    ...
    /**
     * Shutdown the WFA Swarm Queue system gracefully
     */
    async shutdown() {
        try {
            console.log('🔄 Shutting down WFA Swarm Queue...');
            
            // Wait for active processors to complete
            const activeTaskIds = Array.from(this.activeProcessors.keys());
            if (activeTaskIds.length > 0) {
                console.log(`⏳ Waiting for ${activeTaskIds.length} active tasks to complete...`);
                
                // Wait up to 30 seconds for active tasks to complete
                const maxWaitTime = 30000;
                const startTime = Date.now();
                
                while (this.activeProcessors.size > 0 && (Date.now() - startTime) < maxWaitTime) {
                    await new Promise(resolve => setTimeout(resolve, 1000));
                }
                
                if (this.activeProcessors.size > 0) {
                    console.warn(`⚠️ ${this.activeProcessors.size} tasks still active after shutdown timeout`);
                }
            }
            ...
            console.log('✅ WFA Swarm Queue shutdown completed');
            return true;
            
        } catch (error) {
            console.error('❌ Error during WFA Swarm Queue shutdown:', error);
            throw error;
        }
    }
}
