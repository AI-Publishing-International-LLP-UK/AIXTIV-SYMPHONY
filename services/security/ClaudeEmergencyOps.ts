...
export class ClaudeEmergencyOps {
  ...
  static async restorePlan(opts: RestoreOptions) {
    ...
    if (opts.mode === "ghost") {
      Logger.info("Ghost mode active: simulation only, no live reactivation");
      return graph;
    }
  ...
  private static rebaseTimestamps(graph: any) {
    const now = Date.now();
    const delta = now - new Date(graph.timestamp).getTime();
    graph.tasks = graph.tasks.map((task: any) => ({
      ...task,
      scheduledFor: new Date(new Date(task.scheduledFor).getTime() + delta).toISOString(),
    }));
    graph.timestamp = new Date().toISOString();
    return graph;
  }

  static registerOverrideRole(role: string) {
