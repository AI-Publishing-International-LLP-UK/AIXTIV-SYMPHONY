...
abstract class BaseGovernanceModel implements GovernanceModel {
  ...
  validateActionParameters(
    ...
    if (this.actionLimitations[action]) {
      ...
      if (
        limitations.maxAmount &&
        parameters.amount &&
        parameters.amount > limitations.maxAmount
      ) {
        violations.push(
          `Amount exceeds maximum allowed (${parameters.amount} > ${limitations.maxAmount})`
        );
      }
      ...
      if (
        limitations.maxSize &&
        parameters.size &&
        parameters.size > limitations.maxSize
      ) {
        violations.push(
          `Size exceeds maximum allowed (${parameters.size} > ${limitations.maxSize})`
        );
      }

      // Check allowed types
      if (
        limitations.allowedTypes &&
        parameters.type &&
        !limitations.allowedTypes.includes(parameters.type)
      ) {
        violations.push(
          `Type "${parameters.type}" is not allowed. Allowed types: ${limitations.allowedTypes.join(', ')}`
        );
      }

      // Check restricted categories
      if (
        limitations.restrictedCategories &&
        parameters.category &&
        limitations.restrictedCategories.includes(parameters.category)
      ) {
...
export class EnterpriseGovernanceModel extends BaseGovernanceModel {
  ...
  async canPerformAction(
    ...
    for (const role of userRoles) {
      if (
        this.rolePermissions.roles[role] &&
  ...
  'research-approval': {
    steps: [
      ...
      },
      {
        ...
        conditionalInclude: {
          condition:
            'involvesHumanSubjects == true || involvesAnimalSubjects == true',
        },
      },
    ],
  },
