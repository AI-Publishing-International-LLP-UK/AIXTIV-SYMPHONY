...
const verifyEntitlement = requiredEntitlements => {
  return (req, res, next) => {
    ...
    const userEntitlements =
      subscriptionEntitlements[req.user.subscriptionTier] || [];

    // Check if user has all required entitlements
    const hasAllEntitlements = entitlements.every(entitlement =>
    ...
    if (!hasAllEntitlements) {
      return res.status(403).json({
        ...
        message:
          'Your current subscription does not include access to this resource',
        status: 403,
...
const getMinimumRequiredSubscription = requiredEntitlements => {
  ...
  const needsEnterprise = requiredEntitlements.some(e =>
    ['crx_advanced', 'dream_commander', 'enterprise_wings'].includes(e)
  );

  if (needsEnterprise) return 'enterprise';

  // Check if any premium entitlements are required
  const needsPremium = requiredEntitlements.some(e =>
    ['premium_courses', 'crx_basic'].includes(e)
  );
