...
class GatewayFactory {
    ...
    _initializeGateway(tierType, options) {
        ...
        switch (tierType.toLowerCase()) {
            case TIER_TYPES.OWNER_SUBSCRIBER:
                this.logger.info(`Initializing Owner Subscriber Gateway for user: ${userId}`);
            ...
            case TIER_TYPES.PRACTITIONER:
                this.logger.info(`Initializing Practitioner Gateway for user: ${userId}`);
    ...
    _loadTierSecrets(tierType) {
        try {
            ...
            throw new Error(`Secret initialization failed for tier ${tierType}: ${error.message}`);
        }
    }
