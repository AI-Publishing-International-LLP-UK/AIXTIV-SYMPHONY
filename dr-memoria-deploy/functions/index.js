...
exports.certifyContent = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const result = await contentService.certifyContent(contentId, context.auth.uid);
...
exports.updateContentAnalytics = functions.https.onCall(async (data, context) => {
...
exports.searchContent = functions.https.onCall(async (data, context) => {
  try {
    ...
    if (query) {
      ...
      contentQuery = contentQuery.orderBy('title')
        .startAt(query)
        .endAt(query + '\uf8ff');
    }
...
exports.updateCIGCertifications = functions.pubsub.schedule('every 24 hours').onRun(async (context) => {
  try {
    ...
    snapshot.forEach(doc => {
      ...
      const updateData = {
        'certification.renewedAt': admin.firestore.FieldValue.serverTimestamp(),
        'certification.expiresAt': new Date(Date.now() + 365 * 24 * 60 * 60 * 1000)
      };
      ...
      updatePromises.push(
        admin.firestore()
          ...
          .catch(error => {
            logger.error(`Error renewing certification for content ${doc.id}:`, error);
            return null;
          })
      );
    });
...
exports.handleRevenueSplitEvent = functions.firestore
  ...
  .onCreate(async (snapshot, context) => {
    try {
      ...
      logger.info(`Processing revenue split for content: ${contentId}, transaction: ${transactionId}`);
      
      if (!transactionData.amount || transactionData.processed) {
        logger.warn(`Invalid transaction data or already processed: ${transactionId}`);
        return null;
      }
