...
const nextExport = args => {
  ...
  const options = {
    ...
    outdir: args['--outdir']
      ? (0, _path.resolve)(args['--outdir'])
      : (0, _path.join)(dir, 'out'),
    hasOutdirFromCli: Boolean(args['--outdir']),
    isInvokedFromCli: true,
    hasAppDir: false,
    buildExport: false,
  };
  (0, _export.default)(dir, options, nextExportCliSpan)
    .then(() => {
      nextExportCliSpan.stop();
      (0, _utils.printAndExit)(
        `Export successful. Files written to ${options.outdir}`,
        0
      );
    })
    .catch(err => {
      nextExportCliSpan.stop();
      if (
        err instanceof _export.ExportError ||
