...
async function synthesizeDrBurbyVoice(text, options = {}) {
  ...
  const speakingRate =
    options.speakingRate !== undefined ? options.speakingRate : 0.9;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Voice synthesis failed: ${response.status} ${response.statusText}\n${errorText}`
      );
    }
 ...
 */
async function downloadDrBurbyVoice(
  text,
  filename = 'dr-burby-speech.mp3',
  options = {}
) {
  try {
    const audioBlob = await synthesizeDrBurbyVoice(text, options);
 ...
 */
async function checkDrBurbyVoiceServiceStatus(
  apiEndpoint = 'https://us-central1-api-2100-cool.cloudfunctions.net'
) {
  try {
