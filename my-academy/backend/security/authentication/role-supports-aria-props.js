...
var _getImplicitRole = _interopRequireDefault(
  require('../util/getImplicitRole')
);
...
var errorMessage = function errorMessage(attr, role, tag, isImplicit) {
  if (isImplicit) {
    return 'The attribute '
      .concat(attr, ' is not supported by the role ')
      .concat(role, '. This role is implicit on the element ')
      .concat(tag, '.');
  }
  return 'The attribute '
    .concat(attr, ' is not supported by the role ')
  ...
  create(context) {
    ...
    return {
      JSXOpeningElement(node) {
        ...
        var roleValue = role
          ? (0, _jsxAstUtils.getLiteralPropValue)(role)
        ...
        if (
          ...
          _ariaQuery.roles.get(roleValue) === undefined
        ) {
          return;
        }
        ...
        var invalidAriaPropsForRole = new Set(
          _ariaQuery.aria.keys().filter(function (attribute) {
        ...
        node.attributes
          .filter(function (prop) {
            return (
              (0, _jsxAstUtils.getPropValue)(prop) != null && // Ignore the attribute if its value is null or undefined.
