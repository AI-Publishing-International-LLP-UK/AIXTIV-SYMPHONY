...
export class S2DOManager {
    ...
    try {
      ...
      if (encryptionLevel !== S2DOEncryptionLevel.NONE) {
        ...
        storedData = {
          ...
          data: encryptedData,
          schema:
            typeof data === 'object' ? this.getObjectSchema(data) : undefined,
          version: '1.0',
        };
      } else {
        storedData = {
      ...
      if (
        ...
        if (ownerAddress) {
          try {
            ...
            const { objectHash, transactionId } =
              await this.securityManager.registerObject(
                objectId,
                {
                  id: objectId,
                  ownerType,
            ...
            console.error(
              'Error registering S2DO object on blockchain:',
              error
            );
  ...
  public async updateObjectData<T>(
    ...
    try {
      ...
      const updatedMetadata = {
        ...
        size: JSON.stringify(data).length,
        version: (
          parseFloat(objectMetadata.metadata.version || '1.0') + 0.1
        ).toFixed(1),
      };
  ...
  public async updateObjectPermissions(
    ...
    try {
      ...
      if (
        (objectMetadata.ownerType === 'user' &&
          objectMetadata.ownerId !== userId) ||
        (objectMetadata.ownerType === 'organization' &&
          !(await this.isOrganizationAdmin(objectMetadata.ownerId, userId)))
      ) {
      ...
      if (accessLevel) {
        updatedPermissions.publicAccess =
    ...
    try {
      ...
      if (
        ...
        (ownerType === 'organization' &&
          !(await this.isOrganizationMember(ownerId, userId)))
      ) {
  ...
  public async getAccessibleObjects(
    ...
    try {
      ...
      const publicObjects = publicObjectsSnapshot.docs.map(
        doc => doc.data() as S2DOObject
      );
      ...
      const userObjects = userObjectsSnapshot.docs.map(
        doc => doc.data() as S2DOObject
      );
      ...
      for (const orgId of orgMemberships) {
        ...
        orgObjects.push(
          ...orgObjectsSnapshot.docs.map(doc => doc.data() as S2DOObject)
        );
      }
  ...
  public async searchObjects(
    ...
    try {
      ...
      return accessibleObjects.filter(obj => {
        // Owner type and ID
        if (searchParams.ownerType && obj.ownerType !== searchParams.ownerType)
          return false;
        if (searchParams.ownerId && obj.ownerId !== searchParams.ownerId)
          return false;
  ...
  private encryptData(
    ...
    if (encryptionLevel === S2DOEncryptionLevel.STANDARD) {
      ...
      const key1 = CryptoJS.PBKDF2(key, CryptoJS.lib.WordArray.random(16), {
        keySize: 8,
        iterations: 1000,
      }).toString();
      const key2 = CryptoJS.PBKDF2(key, CryptoJS.lib.WordArray.random(16), {
        keySize: 8,
        iterations: 2000,
      }).toString();
      ...
      const key1 = CryptoJS.PBKDF2(key, CryptoJS.lib.WordArray.random(32), {
        keySize: 16,
        iterations: 2000,
      }).toString();
      const key2 = CryptoJS.PBKDF2(key, CryptoJS.lib.WordArray.random(32), {
        keySize: 16,
        iterations: 3000,
      }).toString();
      const key3 = CryptoJS.PBKDF2(key, CryptoJS.lib.WordArray.random(32), {
        keySize: 16,
        iterations: 4000,
      }).toString();
      ...
      const secondPass = CryptoJS.TripleDES.encrypt(firstPass, key1, {
        salt,
      }).toString();

      // Third round with AES again
      const thirdPass = CryptoJS.AES.encrypt(secondPass, key2, {
        mode: CryptoJS.mode.CBC,
      }).toString();

      // Final round
      return CryptoJS.AES.encrypt(thirdPass, key3, {
  ...
  private decryptData(
    ...
    try {
      if (encryptionLevel === S2DOEncryptionLevel.STANDARD) {
        ...
        const firstPass = CryptoJS.AES.decrypt(encryptedData, key2).toString(
          CryptoJS.enc.Utf8
        );
        const secondPass = CryptoJS.AES.decrypt(firstPass, key1).toString(
          CryptoJS.enc.Utf8
        );
        return CryptoJS.AES.decrypt(secondPass, key).toString(
          CryptoJS.enc.Utf8
        );
      } else if (encryptionLevel === S2DOEncryptionLevel.ULTRA) {
        ...
        const firstPass = CryptoJS.AES.decrypt(encryptedData, key3, {
          mode: CryptoJS.mode.CTR,
        }).toString(CryptoJS.enc.Utf8);
        const secondPass = CryptoJS.AES.decrypt(firstPass, key2, {
          mode: CryptoJS.mode.CBC,
        }).toString(CryptoJS.enc.Utf8);
        const thirdPass = CryptoJS.TripleDES.decrypt(secondPass, key1).toString(
          CryptoJS.enc.Utf8
        );
  ...
  private async getEncryptionKey(
    ...
    try {
      ...
      const keyDoc = await getDoc(
        doc(db, 's2doEncryptionKeys', `key_${objectId}`)
      );
