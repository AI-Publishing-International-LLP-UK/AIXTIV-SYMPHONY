var fs = require('fs'),
  { join: joinPath, parse: parsePath, resolve: resolvePath } = require('path'),
  _ = require('lodash'),
  async = require('async'),
    ...
    fs.writeFile(path.unparsed, content, function (err) {
      cb(
        _.set(
          err,
          'help',
          `error writing file "${path.unparsed}" for ${options.name || 'unknown-source'}`
        ),
        path
      );
    });
  ...
  timestamp = function (date, separator) {
    ...
    return (date || new Date())
      .toISOString()
      .replace(/[^\d]+/g, _.isString(separator) ? separator : TS_SEP);
  };
...
module.exports = function (options, done) {
  ...
  var path = _.isString(options.path) && parsePath(resolvePath(options.path)),
    content = _.isPlainObject(options.content)
      ? JSON.stringify(options.content, 0, 2)
      : options.content || E;
