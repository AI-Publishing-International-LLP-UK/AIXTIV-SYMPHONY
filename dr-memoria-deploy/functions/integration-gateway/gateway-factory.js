...
class GatewayFactory {
  ...
  constructor(options = {}) {
    ...
    logger.info('GatewayFactory initialized with cache enabled:', this.settings.enableCaching);
  }
  ...
  createGateway(tierType, context, options = {}) {
    ...
    try {
      ...
      const gatewayInstance = this._instantiateGateway(normalizedTierType, context, options);
      ...
      throw new Error(`Failed to create gateway for tier ${tierType}: ${error.message}`);
    }
  }
  ...
  clearCache(tierType) {
    if (tierType) {
      ...
      logger.info(`Cleared ${clearedCount} cache entries for tier type: ${normalizedTierType}`);
      return clearedCount;
    } else {
      // Clear all entries
