...
/**
 * Configuration interface for all connectors
 */
export interface ConnectorConfig {
  ...
  authType?: 'none' | 'basic' | 'token' | 'oauth2' | 'custom';
  credentials?: {
    username?: string;
    password?: string;
    apiKey?: string;
...
export abstract class BaseConnector extends EventEmitter {
   ...
   */
  public getLastError(): ConnectorError | null {
  ...
  protected logActivity(
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      console[logMethod](`[${entry.timestamp.toISOString()}] [${this.config.name}] ${message}`, 
        details ? { ...details, correlationId } : correlationId ? { correlationId } : undefined
      );
    }
  }
   ...
   */
  public getRetryConfig(): typeof this.config.retryOptions {
