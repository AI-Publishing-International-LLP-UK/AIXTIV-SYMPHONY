...
export class ConfidenceScoreCalculator {
  ...
  private calculateScoreComponents(
    ...
    return {
      ...
      collaborativeCapacity: this.calculateCollaborativeCapacity(
        profileData,
        drMatchData
      ),
   ...
   */
  public calculateComprehensiveScore(params: {
    ...
    if (params.additionalFactors) {
      ...
      return {
        ...
        domainConfidence: Math.min(
          1.0,
          domainConfidence * (1 + domainAdjustment)
        ),
        profileAuthenticity: Math.min(
          1.0,
