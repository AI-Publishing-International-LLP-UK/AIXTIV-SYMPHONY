...
function validateServiceAccountKey(filePath) {
  ...
  try {
    ...
    for (const field of requiredFields) {
      if (!keyData[field]) {
        console.error(chalk.red(`Invalid service account key: missing ${field}`));
        return false;
      }
    }
    ...
    if (keyData.type !== 'service_account') {
      console.error(chalk.red(`Invalid key type: ${keyData.type}. Expected: service_account`));
      return false;
    }
    
    console.log(chalk.green(`✅ Valid service account key for: ${keyData.client_email}`));
    return true;
  } catch (error) {
    console.error(chalk.red(`Error validating service account key: ${error.message}`));
    return false;
  }
}
...
function backupCurrentKey(currentKeyPath) {
  ...
  try {
    ...
    const backupPath = path.join(CONFIG.backupDir, `service-account-key.${timestamp}.json.bak`);
...
function updateServiceAccountKey(newKeyPath) {
  ...
  // Validate parameters
  if (!newKeyPath) {
    console.error(chalk.red('Error: No path provided for the new service account key.'));
    console.log(chalk.yellow('Usage: node service-account-key-manager.js [path-to-new-key-file]'));
    process.exit(1);
  }
  ...
  if (!isGitWorkingDirClean()) {
    console.error(chalk.red('Error: Git working directory is not clean. Please commit or stash changes before updating the key.'));
    process.exit(1);
  }
  ...
  if (currentKeyHash && currentKeyHash === newKeyHash) {
    console.log(chalk.yellow('⚠️ The new key is identical to the current key. No update needed.'));
    process.exit(0);
  }
  ...
  try {
    ...
    if (fs.existsSync(zshrcPath)) {
      ...
      if (envVarRegex.test(zshrcContent)) {
        ...
        console.log(chalk.green('✅ Updated GOOGLE_APPLICATION_CREDENTIALS in .zshrc'));
      }
    }
  } catch (error) {
    console.warn(chalk.yellow(`Warning: Could not update .zshrc: ${error.message}`));
  }
  ...
  console.log(chalk.blue('to refresh your environment variables in the current terminal session.'));
}
