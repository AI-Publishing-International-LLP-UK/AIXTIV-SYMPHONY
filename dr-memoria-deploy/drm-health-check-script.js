...
program
  ...
  .option(
    ...
    'Comma-separated function names to check (default: all)'
  )
...
function initializeFirebase() {
  try {
    ...
    log.info(
      `Initialized Firebase Admin SDK for project ${admin.app().options.projectId}`
    );
    return true;
  } catch (error) {
...
async function listFunctions() {
  try {
    ...
    log.info(
      `Getting functions metadata from Firebase project ${projectId} in region ${region}`
    );
...
function checkHttpFunction(functionData) {
  return new Promise(resolve => {
    ...
    log.info(
      `Checking HTTP function: ${functionData.name} at ${functionData.url}`
    );
    ...
    const request = https.get(functionData.url, { timeout }, response => {
      ...
      if (statusCode >= 200 && statusCode < 500) {
        log.info(
          `Function ${functionData.name} responded with status ${statusCode}`
        );
        resolve({ name: functionData.name, healthy: true, statusCode });
      } else {
        log.warn(
          `Function ${functionData.name} responded with error status ${statusCode}`
        );
    ...
    request.on('error', error => {
      log.error(
        `Error checking function ${functionData.name}: ${error.message}`
      );
      resolve({
    ...
    request.on('timeout', () => {
      log.error(
        `Timeout checking function ${functionData.name} after ${timeout}ms`
      );
      request.destroy();
...
async function checkNonHttpFunction(functionData) {
  try {
    ...
    if (functionRef) {
      log.info(
        `Function ${functionData.name} exists and appears to be deployed`
      );
...
async function main() {
  ...
  try {
    ...
    if (options.functions) {
      ...
      functionsToCheck = allFunctions.filter(fn =>
        functionNames.includes(fn.name)
      );
      log.info(
        `Filtered to ${functionsToCheck.length} functions for health check`
      );
    }
    ...
    if (unhealthy.length > 0) {
      ...
      unhealthy.forEach(check =>
        log.error(
          `  ✗ ${check.name}: ${check.error || check.statusCode || 'unknown error'}`
        )
      );
    }
