...
const loggerMiddleware = (options = {}) => {
  ...
  return (req, res, next) => {
    ...
    req.log = {
      info: (message) => console.log(JSON.stringify(formatLogMessage(req, message, 'info'))),
      warn: (message) => console.warn(JSON.stringify(formatLogMessage(req, message, 'warn'))),
      error: (message, error) => {
    ...
    res.end = function(chunk, encoding) {
      ...
      // Log the response
      const logObj = formatLogMessage(req, `Request completed: ${req.method} ${req.path}`, 'info');
      logObj.response = {
