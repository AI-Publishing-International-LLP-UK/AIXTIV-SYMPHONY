...
const SD20Dashboard = () => {
  ...
  const renderPriority = priority => {
    ...
    return (
      <span
        className={`text-xs font-medium px-2 py-1 rounded-full ${colors[priority]}`}
      >
  ...
  const renderAvatar = initiator => {
    return (
      <div className="relative">
        ...
        {initiator.isAgent && (
          <div
            className="absolute -right-1 -bottom-1 w-4 h-4 bg-blue-500 rounded-full border-2 border-white"
            title="AI Agent"
          ></div>
        )}
      </div>
    ...
    <div className="max-w-4xl mx-auto p-4 bg-white rounded-lg shadow">
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Manage verification requests and completed actions
        </p>
      </div>
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-gray-800">
          {activeTab === 'pending'
            ? 'Actions Requiring Your Verification'
            : activeTab === 'completed'
          ...
          {pendingActions.map(action => (
            ...
            >
              ...
              <div className="flex justify-between text-sm text-gray-500">
                ...
                <span>
                  {new Date(action.metadata.createdAt).toLocaleTimeString()} ·{' '}
          ...
          <div className="bg-white rounded-lg max-w-lg w-full p-6">
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-xl font-bold text-gray-900">
                {formatAction(selectedAction.action)}
              </h3>
              <button
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div>
                ...
                <p className="text-gray-600">
                  {selectedAction.metadata.domain}
                </p>
              </div>
              <div>
                <p className="font-medium text-gray-800">Created</p>
                <p className="text-gray-600">
                  {new Date(selectedAction.metadata.createdAt).toLocaleString()}
                </p>
              </div>
              <div>
                <p className="font-medium text-gray-800">Priority</p>
                <p className="text-gray-600">
                  {renderPriority(selectedAction.metadata.priority)}
                </p>
              </div>
            </div>
