...
export abstract class BaseIntegrationGateway {
  ...
  protected abstract handleSpecificAuthorization(
    req: ExtendedRequest,
    res: Response,
    next: NextFunction
  ): void;
...
export class OwnerIntegrationGateway extends BaseIntegrationGateway {
  ...
  protected handleSpecificAuthorization(
    req: ExtendedRequest,
    res: Response,
    next: NextFunction
  ): void {
...
export class EnterpriseIntegrationGateway extends BaseIntegrationGateway {
  ...
  protected setupSpecificEndpoints(): void {
    ...
    this.app.get(
      '/organization',
      async (_req: ExtendedRequest, res: Response) => {
        try {
          const result = await db.query(
            'SELECT * FROM organizations WHERE id = $1',
            [this.organizationId]
...
export class IntegrationGatewayFactory {
  static async createGateway(
    ...
    try {
      ...
      switch (gatewayType) {
        case GatewayType.OWNER:
          gatewayInstance = new OwnerIntegrationGateway(
            gateway.id,
            ownerId,
            ownerType
          );
        ...
        case GatewayType.ENTERPRISE:
          ...
          break;
        case GatewayType.OWNER_SUBSCRIBER:
          gatewayInstance = new OwnerSubscriberIntegrationGateway(
            gateway.id,
            ownerId,
            ownerType
          );
