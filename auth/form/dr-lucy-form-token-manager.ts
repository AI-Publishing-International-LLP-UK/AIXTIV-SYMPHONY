   ...
   */
  async validateTokenWithCsrf(token: string, csrfToken: string, sessionId: string): Promise<boolean> {
...
export {
  ...
  FormOriginValidator
      ...
      if (req.query && req.query.form_token) {
        return req.query.form_token as string;
      }
   ...
   * Revoke a form token
   */
  async generateToken(payload: FormTokenPayload): Promise<FormToken> {
    try {
   ...
   * Cache a token
   */
  private cacheToken(formToken: FormToken): void {
