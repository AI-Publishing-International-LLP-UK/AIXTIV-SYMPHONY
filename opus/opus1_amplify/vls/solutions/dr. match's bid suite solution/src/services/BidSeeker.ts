...
export class BidSeeker {
  ...
  async searchBids(
    ...
    const queryConstraints: QueryConstraint[] =
      this.buildQueryConstraints(criteria);
    ...
    const personalizedBids = context
      ? this.personalizeResults(bids, context)
      : bids;
  ...
  private buildQueryConstraints(
    ...
    if (criteria.responderId) {
      constraints.push(
        where('responses.responderId', '==', criteria.responderId)
      );
    }
    ...
    if (criteria.location) {
      if (criteria.location.country) {
        constraints.push(
          where('location.country', '==', criteria.location.country)
        );
      }
      ...
      if (criteria.location.remote !== undefined) {
        constraints.push(
          where('location.remote', '==', criteria.location.remote)
        );
      }
