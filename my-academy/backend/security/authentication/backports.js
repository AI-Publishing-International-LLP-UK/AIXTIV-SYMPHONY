...
var context =
  ((_a = {}), (_a[bs_logger_1.LogContexts.namespace] = 'backports'), _a);
/**
 * @internal
 */
var backportJestConfig = function (logger, config) {
  logger.debug(
    __assign(__assign({}, context), { config: config }),
    'backporting config'
  );
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  var _a = (config || {}).globals,
    globals = _a === void 0 ? {} : _a;
  var _b = globals['ts-jest'],
    tsJest = _b === void 0 ? {} : _b;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  var mergeTsJest = {};
  ...
  var warnConfig = function (oldPath, newPath, note) {
    ...
    logger.warn(
      ...
      (0, messages_1.interpolate)(
        note
          ? '"[jest-config].{{oldPath}}" is deprecated, use "[jest-config].{{newPath}}" instead.\n    \u21B3 {{note}}' /* Deprecations.ConfigOptionWithNote */
          : '"[jest-config].{{oldPath}}" is deprecated, use "[jest-config].{{newPath}}" instead.' /* Deprecations.ConfigOption */,
        {
          oldPath: oldPath,
          newPath: newPath,
          note: note,
  ...
  if ('useBabelrc' in tsJest) {
    warnConfig(
      ...
      'See `babel-jest` related issue: https://github.com/facebook/jest/issues/3845' /* Deprecations.ConfigOptionUseBabelRcNote */
    );
  ...
  if (hadWarnings) {
    logger.warn(
      context,
      'Your Jest configuration is outdated. Use the CLI to help migrating it: ts-jest config:migrate <config-file>.' /* Helps.MigrateConfigUsingCLI */
    );
  }
  return __assign(__assign({}, config), {
    globals: __assign(__assign({}, globals), {
      'ts-jest': __assign(__assign({}, mergeTsJest), tsJest),
    }),
  });
};
...
var backportTsJestDebugEnvVar = function (logger) {
  if ('TS_JEST_DEBUG' in process.env) {
    var shouldLog = !/^\s*(?:0|f(?:alse)?|no?|disabled?|off|)\s*$/i.test(
    ...
    logger.warn(
      context,
      (0, messages_1.interpolate)(
        'Using env. var "{{old}}" is deprecated, use "{{new}}" instead.' /* Deprecations.EnvVar */,
        {
          old: 'TS_JEST_DEBUG',
          new: 'TS_JEST_LOG',
        }
