  ...
  async (req, res) => {
    try {
      ...
      try {
        ...
        res.send(
          `Hello, authenticated user ${decodedToken.email || decodedToken.uid}!`
        );
      } catch (error) {
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        // Call the TTS service
        const result = await ttsService.ssmlToSpeech(ssml, {
          voice,
          audioConfig,
          storagePath,
        });
        ...
        if (!result.success) {
          ...
          res
            .status(500)
            .send({
              error: 'SSML-to-speech conversion failed',
              details: result.error,
            });
          return;
        }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        res
          .status(500)
          .send({ error: 'Failed to list TTS voices', details: result.error });
        return;
      }
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          'Error verifying ID token or analyzing sentiment:',
          error
        );
