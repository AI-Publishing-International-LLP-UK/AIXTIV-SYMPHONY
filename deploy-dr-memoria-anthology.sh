#!/bin/bash
# Deployment script for Dr. Memoria's Anthology

set -e  # Exit on error

echo "🚀 Starting deployment for Dr. Memoria's Anthology..."
...
firebase target:apply hosting drlucy-live drlucy-live

# Source directory for Dr. Memoria's Anthology
SOURCE_DIR="dr-memoria-deploy/functions"
PUBLIC_DIR="public/dr-memoria-anthology"
...
<head>
  ...
  <title>Dr. Memoria's Anthology</title>
  <style>
    body {
...
# Skipping function deployment for now since we need to resolve conflicts
echo "Skipping Firebase functions deployment to avoid conflicts..."
# cd dr-memoria-deploy/functions
# npm install
# firebase deploy --only functions --project api-for-warp-drive

# Deploy hosting - stay in the root directory
echo "Deploying hosting for Dr. Memoria's Anthology..."
firebase deploy --only hosting:drlucy-live --project api-for-warp-drive

echo "✅ Deployment complete!"
echo "Your Dr. Memoria's Anthology site should now be accessible at:"
echo "  - Firebase URL: https://drlucy-live.web.app"
...
echo ""
echo "To check your deployment, visit: https://dr-memoria.web.app"