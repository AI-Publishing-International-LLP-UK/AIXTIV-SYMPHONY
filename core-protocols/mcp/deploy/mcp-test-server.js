...
let domainIntegration = null;
try {
  domainIntegration = require('./mcp-domain-integration');
  console.log('Domain integration loaded successfully');
} catch (error) {
  console.warn('Domain integration not available:', error.message);
}

// Configuration
const PORT = process.env.PORT || 3000;
const AUTH_TOKEN = process.env.AUTH_TOKEN || 'oauth2'; // Fallback token for development
...
// Integrate domain management resources if available
if (domainIntegration) {
  MCP_CONFIG.resources = [
    ...MCP_CONFIG.resources,
    ...domainIntegration.resources,
  ];

  MCP_CONFIG.tools = [...MCP_CONFIG.tools, ...domainIntegration.tools];

  MCP_CONFIG.prompts = [...MCP_CONFIG.prompts, ...domainIntegration.prompts];

  console.log('Domain management components integrated with MCP configuration');
}
...
async function startServer() {
  try {
    ...
    // Create HTTP server for health checks and OAuth2 callbacks
    ...
    const server = http.createServer((req, res) => {
      ...
      } else if (req.url.startsWith('/oauth2/callback')) {
        // OAuth2 callback endpoint (for future use)
        res.writeHead(200, { 'Content-Type': 'text/html' });
        res.end(
          '<html><body><h1>OAuth2 Authentication Successful</h1><p>You can close this window now.</p></body></html>'
        );
      } else {
        // Not found
    ...
    wss.on('connection', async (ws, req) => {
      ...
      const isAuthorized = await oauth2Config.validateToken(authHeader);

      if (!isAuthorized) {
        console.warn('Unauthorized connection attempt:', req.headers);
      ...
      ws.on('message', async message => {
        try {
          ...
          logEvent(sessionLog, 'message_received', {
            type: msg.type,
    ...
    function handleListResources(ws, msg, log) {
      ...
      logEvent(log, 'resources_list_sent', {
        count: MCP_CONFIG.resources.length,
      });
    }
    ...
    async function handleExecuteTool(ws, msg, sessionId, log) {
      ...
      try {
        ...
        if (toolId.startsWith('domain/') && domainIntegration) {
          switch (toolId) {
            case 'domain/verify':
              result = await domainIntegration.executeDomainVerification(
                params,
                sessionId,
                log
              );
            ...
            case 'domain/register':
              result = await domainIntegration.executeDomainRegistration(
                params,
                sessionId,
                log
              );
            ...
            case 'domain/status':
              result = await domainIntegration.executeDomainStatusCheck(
                params,
                sessionId,
                log
              );
            ...
            default:
              throw new Error(`Unknown domain tool: ${toolId}`);
          }
        } else {
        ...
        logEvent(log, 'tool_execution_error', {
          tool_id: toolId,
          error: err.message,
        });
    ...
    function handleRenderPrompt(ws, msg, log) {
      ...
      Object.keys(params).forEach(key => {
        ...
        content.prompt = content.prompt.replace(
    ...
    async function executeVerifyS2DO(params, sessionId, log) {
      ...
      return {
        ...
        expected_result_matched:
          verificationResult.result === params.expected_result,
        timestamp: new Date().toISOString(),
      };
    }
    ...
    function verifyS2DOAction(params) {
      ...
      if (!validStems.includes(stem)) {
        return {
          ...
          messages: [
            `Invalid stem: ${stem}. Valid stems are: ${validStems.join(', ')}`,
          ],
        };
      }
      ...
      if (!validActions[stem].includes(action)) {
        return {
          result: 'validation_error',
          messages: [
            `Invalid action: ${action} for stem: ${stem}. Valid actions are: ${validActions[stem].join(', ')}`,
          ],
        };
      }
    ...
    async function executeSimulateCoPilot(params, sessionId, log) {
      ...
      // Find the scenario
      const resourceContent = MCP_CONFIG.resources.find(
        r => r.id === 'test/copilot_scenarios'
      )?.content;
      ...
      const scenario = resourceContent.scenarios.find(
        s => s.id === params.scenario_id
      );
      ...
      logEvent(log, 'simulate_copilot_completed', {
      ...
      return {
        ...
        response: response,
        next_step_index:
          stepIndex < scenario.steps.length - 1 ? stepIndex + 1 : null,
        timestamp: new Date().toISOString(),
      };
    }
    ...
    async function executeValidateDataAccess(params, sessionId, log) {
      ...
      if (
        ...
        params.expected_access === undefined
      ) {
        throw new Error(
          'Missing required parameters for data access validation'
        );
      }

      // Perform data access validation
      const validationResult = validateDataAccess(
        params.data_type,
        params.access_context
      );
    ...
    async function executeLogResult(params, sessionId, log) {
      ...
      const resultsPath = path.join(
        LOG_DIRECTORY,
        `test_result_${params.test_id}.json`
      );
    ...
    async function executeGenerateReport(params, sessionId, log) {
      ...
      if (!params.format) {
        throw new Error(
          'Missing required parameters for generating test report'
        );
      }
      ...
      return {
        ...
        summary: {
          ...
          inconclusive: resultsToInclude.filter(
    ...
    function generateTestReport(testResults, format, includeMetrics) {
      ...
      const testStats = {
        ...
        inconclusive: testResults.filter(r => r.result === 'inconclusive')
          .length,
      };

      // Calculate success rate
      testStats.success_rate =
        testStats.total > 0 ? (testStats.pass / testStats.total) * 100 : 0;
    ...
    function finalizeSessionLog(sessionId, log) {
      ...
      const summaryPath = path.join(
        LOG_DIRECTORY,
        `session_${sessionId}_summary.json`
      );
      const summary = {
        sessionId,
