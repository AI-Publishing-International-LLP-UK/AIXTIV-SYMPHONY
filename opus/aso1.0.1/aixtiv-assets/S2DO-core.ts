...
export class SD20Service {
  ...
  async createActionRequest(
    ...
    if (
      actionRequest.verificationRequirement.type !== 'single' ||
      initiator.isAgent
    ) {
  ...
  async verifyAction(
    ...
    if (approved && allRequirementsMet) {
      ...
      if (
        action.metadata.domain === ActionDomain.FINANCE ||
  ...
  private shouldGenerateNFT(action: SD20ActionRecord): boolean {
    ...
    if (
      action.metadata.domain === ActionDomain.CREATIVE &&
    ...
    const metadata = {
      ...
      attributes: [
        ...
        { trait_type: 'Domain', value: action.metadata.domain },
        {
          ...
          value: new Date(action.completedAt).toISOString().split('T')[0],
        },
   ...
   */
  private async executeResultingActions(
    action: SD20ActionRecord
  ): Promise<void> {
  ...
  queryActions(filters: {
    ...
    return Array.from(this.actions.values()).filter(action => {
      ...
      if (filters.initiatorId && action.initiator.id !== filters.initiatorId)
        return false;
      if (filters.domain && action.metadata.domain !== filters.domain)
        return false;
      if (filters.action && !action.action.includes(filters.action))
        return false;
      if (filters.startDate && action.metadata.createdAt < filters.startDate)
        return false;
      if (filters.endDate && action.metadata.createdAt > filters.endDate)
        return false;
      return true;
    });
  }
...
export interface BlockchainService {
  ...
  verifyActionRecord(actionId: string, actionHash: string): Promise<boolean>;
}
