...
describe('S2DOBlockchainService', () => {
  ...
  describe('storeActionVerification', () => {
    it('should store action verification successfully', async () => {
      ...
      // Assert
      expect(mockActionContract.verifyActionHash).toHaveBeenCalledWith(
        actionId,
        actionHash
      );
      ...
      expect(mockActionContract.verifyAction).toHaveBeenCalledTimes(
    ...
    it('should throw an error if action already exists', async () => {
      ...
      // Act & Assert
      await expect(
        service.storeActionVerification(
          'test-action-id',
          'test-hash',
          'initiator-address',
          []
  ...
  describe('getAllActions', () => {
    ...
    it('should return empty array on error', async () => {
      // Arrange
      const spy = jest
        .spyOn(mockActionContract, 'getAllActions')
  ...
  describe('getAction', () => {
    ...
    it('should return null if getting the action fails', async () => {
      // Arrange
      const spy = jest
        .spyOn(mockActionContract, 'getAction')
        .mockRejectedValue(new Error('Test error'));
  ...
  describe('Event Listeners', () => {
    it('should register and remove event listeners', () => {
      ...
      expect(mockActionContract.on).toHaveBeenCalledWith(
        'TestEvent',
        expect.any(Function)
      );
