...
import {
  SERPEWConnector,
  SectorStandardsConnector,
  JobDefinitionsConnector,
...
export class S2DOSystem {
  ...
  constructor(private config: S2DOSystemConfig) {
    ...
    Object.values(UserType).forEach(userType => {
      this.governanceModels.set(
        userType,
        GovernanceFactory.createGovernanceModel(userType)
      );
    });
  ...
  private async deployBlockchainContracts(): Promise<void> {
    try {
      ...
      this.config.blockchain.contracts.actionVerification =
        deployedContracts.actionVerification;
      this.config.blockchain.contracts.achievementNFT =
    ...
    try {
      ...
      if (action) {
        ...
        for (const participant of participants) {
          await this.notifyParticipantOfAchievement(
            participant,
            tokenId.toString(),
            action
          );
  ...
  private async notifyParticipantOfAchievement(
    ...
    console.log(
      `Notifying ${participant.name} of achievement ${tokenId} for action ${action.id}`
    );
  }
   ...
   */
  private async generateNFTForAction(
    action: SD20ActionRecord
  ): Promise<string> {
  ...
  async createAction(
    ...
    const verificationRequirement =
      governanceModel.getVerificationRequirement(action);
 ...
 */
export async function createS2DOSystem(
  config?: Partial<S2DOSystemConfig>
): Promise<S2DOSystem> {
  ...
  const defaultConfig: S2DOSystemConfig = {
    ...
    notification: {
      ...
      fromEmail:
        ...
        'notifications@aixtiv-symphony.coaching2100.com',
