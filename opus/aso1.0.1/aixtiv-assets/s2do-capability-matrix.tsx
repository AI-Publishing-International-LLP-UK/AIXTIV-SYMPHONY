...
const S2DOCapabilityMatrix = () => {
  ...
  phases.forEach(phase => {
    capabilitiesByPhase[phase.id] = filteredCapabilities.filter(
      cap => cap.phase === phase.id
    );
  });
  ...
  domains.slice(1).forEach(domain => {
    // Skip "All Domains"
    capabilitiesByDomain[domain.id] = filteredCapabilities.filter(
      cap => cap.domain === domain.id
    );
  });
    ...
    <div className="p-4 max-w-6xl mx-auto bg-gray-50 rounded-lg shadow-lg">
      <div className="mb-6 text-center">
        ...
        <p className="text-gray-600">
          All capabilities are now active and available across all phases
        </p>
      </div>
          ...
          {activeDomain === 'all' ? (
            ...
            domains.slice(1).map(domain => (
              <div key={domain.id} className="mb-6">
                ...
                {capabilitiesByDomain[domain.id] &&
                capabilitiesByDomain[domain.id].length > 0 ? (
                  <div className="bg-white rounded-b-lg border-x border-b border-gray-200">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {capabilitiesByDomain[domain.id].map(
                          (capability, idx) => (
                            <tr key={idx}>
                              <td className="px-4 py-3 whitespace-nowrap font-medium">
                                {capability.example}
                              ...
                              <td className="px-4 py-3 whitespace-nowrap">
                                ...
                                >
                                  {
                                    phases.find(p => p.id === capability.phase)
                                      ?.name
                                  }
                                </span>
                              </td>
      ...
      <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
        ...
        <p className="mb-3">
          All capabilities from all phases are now available simultaneously.
          Organizations can select and implement specific capabilities based on
          their readiness and requirements, without following a strict
          sequential progression.
        </p>
        <p>
          The S2DO system architecture supports this flexible implementation
          approach, with each capability operating independently while
          maintaining full compatibility with all other capabilities in the
          ecosystem.
        </p>
      </div>
    </div>
