...
const BLOCKCHAIN_RPC = process.env.BLOCKCHAIN_RPC || 'https://eth-mainnet.alchemyapi.io/v2/your-api-key';
const S2DO_CONTRACT_ADDRESS = process.env.S2DO_CONTRACT_ADDRESS || '0x123456789abcdef';
const S2DO_CONTRACT_ABI = require('./contracts/s2do_abi.json');
const FMS_CONTRACT_ADDRESS = process.env.FMS_CONTRACT_ADDRESS || '0xabcdef123456789';
...
const s2doContract = new ethers.Contract(S2DO_CONTRACT_ADDRESS, S2DO_CONTRACT_ABI, wallet);
const fmsContract = new ethers.Contract(FMS_CONTRACT_ADDRESS, FMS_CONTRACT_ABI, wallet);
    ...
    case 's2do/schema':
      resourceData = {
        ...
        properties: {
          ...
          initiator: { type: 'string', description: 'Entity that initiated the action' }
        },
    ...
    tools: [
      ...
      },
      {
        ...
        parameters: {
          type: 'object',
          properties: {
            achievement_type: { type: 'string', enum: ['milestone', 'completion', 'excellence'] },
...
async function executeS2DOMint(params) {
  ...
  const invalidActions = actions.filter(actionId => !s2doVerifications.has(actionId));
