...
var Importer = /** @class */ (function () {
  ...
  Object.defineProperty(Importer, 'instance', {
    get: function () {
      ...
      return new Importer({
        '@babel/core': [
          passThru(version_checkers_1.VersionCheckers.babelCore.warn),
        ],
        'babel-jest': [
          passThru(version_checkers_1.VersionCheckers.babelJest.warn),
        ],
        typescript: [
          passThru(version_checkers_1.VersionCheckers.typescript.warn),
        ],
      ...
      if (req.exists) {
        ...
        if (loaded.error) {
          ...
          logger.error(
            ...
            "failed loading module '".concat(name, "'"),
  ...
  Importer.prototype._patch = function (name, unpatched) {
    if (name in this._patches) {
      ...
      return this._patches[name].reduce(function (mod, patcher) {
        return patcher(mod);
      }, unpatched);
    }
    return unpatched;
  ...
  Importer.prototype._import = function (why, moduleName, _a) {
    var _b = _a === void 0 ? {} : _a,
      _c = _b.alternatives,
      alternatives = _c === void 0 ? [] : _c,
      _d = _b.installTip,
      installTip = _d === void 0 ? moduleName : _d;
    // try to load any of the alternative after trying main one
    var res = this.tryThese.apply(
      this,
      __spreadArray([moduleName], __read(alternatives), false)
    );
    ...
    if (res && res.exists) {
      ...
      throw new Error(
        (0, messages_1.interpolate)(
          'Loading module {{module}} failed with error: {{error}}' /* Errors.LoadingModuleFailed */,
          { module: res.given, error: res.error.message }
        )
      );
    }
    // if it couldn't load, build a nice error message so the user can fix it by himself
    var msg = alternatives.length
      ? 'Unable to load any of these modules: {{module}}. {{reason}}. To fix it:\n{{fix}}' /* Errors.UnableToLoadAnyModule */
      : 'Unable to load the module {{module}}. {{reason}} To fix it:\n{{fix}}'; /* Errors.UnableToLoadOneModule */
    var loadModule = __spreadArray([moduleName], __read(alternatives), false)
      .map(function (m) {
        return '"'.concat(m, '"');
      })
      .join(', ');
    if (typeof installTip === 'string') {
      installTip = [
        { module: installTip, label: 'install "'.concat(installTip, '"') },
      ];
    }
    var fix = installTip
      .map(function (tip) {
        return '    '
          .concat(installTip.length === 1 ? '↳' : '•', ' ')
          .concat(
            (0, messages_1.interpolate)(
              '{{label}}: `npm i -D {{module}}` (or `yarn add --dev {{module}}`)' /* Helps.FixMissingModule */,
              tip
            )
          );
      })
  ...
  __decorate(
    [
      (0, memoize_1.Memoize)(function (name) {
        return name;
      }),
    ],
    Importer.prototype,
  ...
  __decorate([(0, memoize_1.Memoize)()], Importer, 'instance', null);
  return Importer;
})();
...
var requireModule = function (mod) {
  return require(mod);
};
var resolveModule = function (mod) {
  return require.resolve(mod, { paths: [process.cwd(), __dirname] });
};
/**
 * @internal
 */
