...
class Logger {
  ...
  _write(level, levelName, message, data = {}) {
    ...
    if (this.enableConsole) {
      if (this.jsonOutput) {
        ...
        logFn(`[${logObj.timestamp}] [${logObj.level}] [${logObj.component}] [${logObj.tier}] [${logObj.correlation_id}] ${logObj.message}`);
      }
    }
    
    // File output
    if (this.enableFile) {
      const logFile = path.join(this.filePath, `${this.name}-${new Date().toISOString().split('T')[0]}.log`);
  ...
  child(childOptions = {}) {
    return new Logger({
      ...
      console: childOptions.console !== undefined ? childOptions.console : this.enableConsole,
      file: childOptions.file !== undefined ? childOptions.file : this.enableFile,
      filePath: childOptions.filePath || this.filePath,
      json: childOptions.json !== undefined ? childOptions.json : this.jsonOutput,
