...
interface RiasecProfile {
  realistic: number; // Working with things, hands-on
  investigative: number; // Working with ideas, analytical
  artistic: number; // Working with creativity, self-expression
  social: number; // Working with people, helping
  enterprising: number; // Working with people, leadership
  conventional: number; // Working with data, organization
  primaryType: string; // Top personality type
  secondaryType: string; // Second highest type
}
 ...
 */
interface DfabAssessment {
  direction: number; // Strategic orientation
  function: number; // Operational execution 
  action: number; // Tactical implementation
  behavior: number; // Personal conduct and team interaction
  primaryStrength: string; // Top strength area
  developmentArea: string; // Area for improvement
  leadershipStyle: string; // Overall leadership characterization
}
...
interface NineBoxPosition {
  ...
  description: string; // Detailed characterization
 ...
 */
interface CareerExpertiseProfile {
  id: string;
  userId: string;
  riasecProfile: RiasecProfile;
  dfabAssessment: DfabAssessment;
  careerAttributes: {
    industries: {
      primary: IndustryCategory;
      secondary: IndustryCategory;
      experience: Record<string, number>; // Years by industry
    };
    roleLevel: RoleLevel;
    yearsOfExperience: number;
    specializations: string[];
    careerTrajectory: {
      verticalMovement: number; // Positive = upward, Negative = downward
      horizontalMovement: number; // Career changes across industries
      growthRate: number; // Rate of progression
    };
  };
 ...
 */
const NINE_BOX_MAPPINGS: Record<string, NineBoxPosition> = {
  '1,1': {
    x: 1,
    y: 1,
    label: 'Risk',
 ...
 */
const RIASEC_INDUSTRY_MAPPING: Record<string, IndustryCategory[]> = {
...
export const createCareerProfile = functions.https.onCall({
  ...
  try {
    ...
    const profile: Omit<CareerExpertiseProfile, 'createdAt' | 'updatedAt'> & {
    ...
    throw new functions.https.HttpsError(
      'internal',
      'Error creating career profile: ' + (error as Error).message
    );
  }
});
...
export const getCareerProfile = functions.https.onCall({
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      'internal',
      'Error getting career profile: ' + (error as Error).message
    );
  }
});
...
export const updateCareerProfile = functions.https.onCall({
  ...
  try {
    ...
    const existingProfile = profileDoc.data() as CareerExpertiseProfile;
    ...
    const careerAttributes: Record<string, any> = {};
    ...
    if (roleLevel || yearsOfExperience !== undefined) {
      careerAttributes['careerTrajectory'] = calculateCareerTrajectory(
        ...
        yearsOfExperience !== undefined ? yearsOfExperience : existingProfile.careerAttributes.yearsOfExperience
      );
    }
    ...
    throw new functions.https.HttpsError(
      'internal',
      'Error updating career profile: ' + (error as Error).message
    );
  }
});
...
export const getCareerRecommendations = functions.https.onCall({
  ...
  try {
    ...
    const profile = profileDoc.data() as CareerExpertiseProfile;
    ...
    throw new functions.https.HttpsError(
      'internal',
      'Error getting career recommendations: ' + (error as Error).message
    );
  }
});
...
export const getCareerBenchmarks = functions.https.onCall({
  ...
  try {
    ...
    let peerProfiles = snapshot.docs.map(doc => doc.data() as CareerExpertiseProfile);
    ...
    throw new functions.https.HttpsError(
      'internal',
      'Error getting career benchmarks: ' + (error as Error).message
    );
  }
});
 ...
 */
function calculateRiasecProfile(scores: Record<string, number>): RiasecProfile {
  const defaultScores = {
    realistic: 0,
    investigative: 0,
    artistic: 0,
 ...
 */
function calculateDfabAssessment(scores: Record<string, number>): DfabAssessment {
  const defaultScores = {
    direction: 0,
    function: 0,
    action: 0,
 ...
 * Calculate Nine Box position
 */
function calculateNineBoxPosition(
...
function calculateCareerTrajectory(
  ...
  const roleLevelValues: Record<RoleLevel, number> = {
    [RoleLevel.ENTRY]: 1,
...
function calculateBenchmarks(
  ...
  industries: { primary: IndustryCategory; secondary: IndustryCategory; experience: Record<string, number> },
  roleLevel: RoleLevel,
  yearsOfExperience: number
): {
  industryPercentile: number;
 ...
 */
function calculateRiasecIndustryMatch(
  riasecProfile: RiasecProfile,
  ...
  for (const type of topTypes) {
    if (RIASEC_INDUSTRY_MAPPING[type]?.includes(industry)) {
...
function calculateGrowthPotential(
  ...
  const riasecAdjustment = (
    riasecProfile.enterprising * 0.6 +
    riasecProfile.investigative * 0.4
  ) / 100;
 ...
 */
function getSkillsForRiasecArea(area: string): string[] {
  switch (area) {
    case 'realistic':
...
function getIndustrySpecificRoles(
  ...
  const industryRoles: Record<IndustryCategory, Record<string, string[]>> = {
...
function calculateDetailedBenchmarks(
  profile: CareerExpertiseProfile,
  peerProfiles: CareerExpertiseProfile[]
): {
  overall: {
    percentile: number;
    peerCount: number;
  ...
  const byDimension: Record<string, { score: number; percentile: number }> = {};
  ...
  const getLevelName = (value: number): string => {
    for (const [level, levelValue] of Object.entries(roleLevelValues)) {
      if (levelValue === value) {
        return formatRoleLevel(level as RoleLevel);
      }
    }
 ...
 */
function formatRoleLevel(level: RoleLevel): string {
  switch (level) {
